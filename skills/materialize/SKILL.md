...
- **Pipeline gate.** The pipeline for the chosen workflow type (see **Workflow types**) is a contract, not a menu. Record every prescribed phase in the marker `done:` as you finish it; never declare done or open a PR while a prescribed phase is neither done nor logged `skipped: <reason>`. Two phases carry an independent contract the orchestrator cannot self-satisfy: **verify** (a *fresh sub-agent that did not write the code* records a predicate verdict at `.workflow/<id>/NN-verify-*.md`, no open FAILs — the loop-close review does not count) and **accept** (drive the live app through the **browser** slot at PRD scope before a SPEC project is done). Set `verified:`/`accepted:` in the marker; refuse to declare done otherwise.
