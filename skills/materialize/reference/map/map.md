This mode applies when a loose idea requires more than one agent session to turn into a plan. It creates a stateful decision map in a markdown file, and drives the user through a sequence of tickets to resolve the open questions - which may require research, prototyping, or discussion.
...
2. Run a session to resolve the ticket, invoking modes as needed. If in doubt, use `grill` and `model`.
