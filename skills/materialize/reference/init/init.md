...
**Optional — install the mode-injector hook (Claude Code).** Install [hooks/materialize-mode-injector.sh](./hooks/materialize-mode-injector.sh) into `.claude/hooks/` and register it under `hooks.SessionStart` in `.claude/settings.json`. It automatically injects the active phase's instructions into the sub-agent's starting session context, ensuring the sub-agent executes the phase according to the prescribed mode without needing manual file reads.

**Optional — install the mode-enforcer hook (Claude Code).** Install [hooks/materialize-mode-enforcer.sh](./hooks/materialize-mode-enforcer.sh) into `.claude/hooks/` and register it under `hooks.PreToolUse` (matcher `.*`) in `.claude/settings.json`. It blocks any tools (except reading) until the agent has explicitly read the active phase reference file (`skills/materialize/reference/<phase>/<phase>.md`), enforcing that phase constraints are understood.

**Optional — install the conductor-lock hook (Claude Code).** Install [hooks/materialize-conductor-lock.sh](./hooks/materialize-conductor-lock.sh) into `.claude/hooks/` and register it under `hooks.PreToolUse` (matcher `Write|Edit`) in `.claude/settings.json`. It disallows the main conductor session from editing or writing files directly, enforcing the rule that all code modifications must be delegated to sub-agents.
