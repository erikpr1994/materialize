...
Beyond `materialize`, a few standalone skills live in bucket folders under `skills/`:

- `engineering/` — code-work helpers not part of the pipeline (currently just `ask-author`)
- `productivity/` — non-code workflow tools (`teach`, `writing-great-skills`)
- `misc/` — kept around but rarely used
- `in-progress/` — drafts not yet ready to ship

`materialize` and every skill in `engineering/`, `productivity/`, or `misc/` must have a reference in the
top-level `README.md` and an entry in `.claude-plugin/plugin.json`. Skills in `in-progress/` must not appear in
either. Each `README.md` entry links the skill name to its `SKILL.md`. Each bucket folder's `README.md` lists its
skills with a one-line description, name linked to `SKILL.md`.
